...
if __name__ == '__main__':
    ...
    df_train = pd.read_csv('server/data/data.csv')
    positive_word = pd.read_excel('./data/positive_words.xlsx')
    df_feature = pd.read_csv('server/data/featuresdf.csv')
    ...
    def pre_process(df_train: pd.DataFrame, positive_word: pd.DataFrame, df_feature: pd.DataFrame):
        ...
        list_country = [{"AD", "Andorra"},
                        ...
                        {"ZM", "Zambia"},
                        {"ZW", "Zimbabwe"}]
        dict_count = {(full.lower() if len(abv) > len(full) else abv.lower()): (abv if len(abv) > len(full) else full)
                      for
        ...
        country_per_month = df_train.loc[df_train.Date.dt.year == 2017, :].groupby('month')[
            'Region'].nunique().to_frame()
        ...
        type_of_graph = {0: 'Up_Down', 2: 'Down', 3: 'Alone Down', 4: 'Sudden SoarUp', 5: 'Remix', 6: 'Down Mean',
                         8: 'Curve'}
        for key, type_ in type_of_graph.items():
            draw_stream_pos(us_unique_song, key, type_)


    pre_process(df_train, positive_word, df_feature)
