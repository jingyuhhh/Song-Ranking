...
def data_forecase(data:pd.DataFrame,song_feature:pd.DataFrame):
    df_train = data
    ...
    def mdl_shape(mdl, row_info, X_train, name='(1,1,0)x(1,1,0,7)xtrend n Model', exog=None, portion=0.5):
        nrows, num_train = row_info  # 提取行列数据
        result = mdl.fit()
        # 这是是许多时间序列模型库（如Statsmodels或Prophet）提供的拟合方法。
        # 这个方法接受训练数据作为输入，然后通过模型的拟合算法来估计模型的参数，以便使模型能够最好地拟合训练数据。
        # 得到的result是包含多种数据的一个集合
        forecast = result.predict(start=int(nrows * portion), end=num_train, dynamic=True, exog=exog)
        # 然后开始输入测试集，并标记起始和终止位置
        X_train['forecast'] = np.hstack([np.zeros((int(nrows * portion) - 1)), forecast.values])
        # 将之前的数据记为0,预测的数据用新预测的数据展示
    ...
    mdl_shape(mdl2, row_info, X_train, name='(110)x(0117)x [Trend: a+b*(x^3)]')
    ...
    prediction_mdl(mdl2, row_info, X_test, name='Prediction (110)x(0117)xTrend n')

    # 接下来,我们通过动态调整x的过程来寻找到一个
    # 两个效果都不错的x
    # 从歌曲的特征入手
    # TODO :应该是要更改文件路径的,当前这里使用的本地的路径,函数中另一个参数就是这个
    ...
    mdl_shape(mdl2, row_info, X_train, name=' With Exog, (110)x(0117)x[Trend: a*x] At Train', exog=exog.reshape(1, 13))
    X_train, X_test, row_info = get_X(shape_of_you)
    prediction_mdl(mdl2, row_info, X_test, name=' With Exog, Prediction (110)x(0117)x[Trend: a*x] At Test',
