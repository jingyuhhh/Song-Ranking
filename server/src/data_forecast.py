...
def data_forecast(data:pd.DataFrame,song_feature:pd.DataFrame):
    ...
    # 该函数用于训练模型
    def mdl_shape(mdl, row_info, X_train, savename,name='(1,1,0)x(1,1,0,7)xtrend n Model', exog=None, portion=0.5):
...
# 这个是
    def prediction_mdl(mdl, row_info, X_test,savename, name='', exog=None):
        nrows, num_train = row_info
        result = mdl.fit()
    ...
    mdl_shape(mdl1, row_info, X_train,"trend constant a test", name='AR = 1,I = 1,SMA = 1,Periods = 7,Trend = no')
    # 显然效果没那么好,因为后序的趋势是下降的
    # 所以接下来我们增加一个趋势

    X_train, X_test, row_info = get_X(shape_of_you)
    mdl2 = SARIMAX(X_train, trend=[1, 0, 0, 1], order=(1, 1, 0), seasonal_order=(0, 1, 1, 7))
    mdl_shape(mdl2, row_info, X_train,"trend constant a + b_3 test", name='AR = 1,I = 1,SMA = 1,Periods = 7,Trend = a+b*(x^3)')
    # 这样看来,我们的训练模型的效果基本不错了
    # 但是我们还要预测后面的数据,所以接下来是预测部分
    prediction_mdl(mdl2, row_info, X_test,"trend constant a + b_3 forecast", name='Prediction AR = 1,I = 1,SMA = 1,Periods = 7,Trend = a+b*(x^3)')
    ...
    prediction_mdl(mdl2, row_info, X_test, "trend constant a forecast",name='AR = 1,I = 1,SMA = 1,Periods = 7,Trend = a')
    ...
    mdl_shape(mdl2, row_info, X_train,"trend a_x test", name=' With Exog,AR = 1,I = 1,SMA = 1,Periods = 7,Trend = a * x',
              exog=exog.reshape(1, 13))
    X_train, X_test, row_info = get_X(shape_of_you)
    prediction_mdl(mdl2, row_info, X_test,"trend a_x forecast",
